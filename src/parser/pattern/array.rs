...
use crate::{
    ...
    error_diag::{SapDiagnosticSpan, SapParserError, SapParserErrorCode}, parser::Rule,
};

use super::{parse_eclipse_pattern, parse_pattern, Pattern};
...
fn parse_array_pattern_elem(elem: Pair<Rule>) -> Result<SapAST, SapParserError> {
    ...
    match elem.as_rule() {
        ...
        Rule::eclipse_pattern => {
            parse_eclipse_pattern(elem).map(|eclipse_pattern| {
                SapAST {
                    span,
...
pub fn parse_array_pattern(array_pattern: Pair<Rule>) -> Result<SapAST, SapParserError> {
    ...
    for elem in array_pattern.into_inner() {
        ...
        let elem = elem.into_inner().next().expect("Array pattern element should have a child");
...
mod tests {
    ...
    fn test_parse_array_pattern() {
        ...
        for input in inputs {
            let mut array_pattern = crate::parser::SapParser::parse(Rule::array_pattern, input).unwrap();
